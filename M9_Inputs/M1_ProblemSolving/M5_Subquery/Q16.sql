...
Track medicine sales across multiple regions. Each region has multiple sales representatives who sell different medicines. Management wants answers to the following:
...
FROM 
 sales_data
GROUP BY 
 region, sales_rep_name
HAVING 
 SUM(quantity_sold * price_per_unit) = (
 SELECT 
 MAX(SUM(quantity_sold * price_per_unit))
 FROM 
 sales_data sub
 WHERE 
 sub.region = sales_data.region
 GROUP BY 
 sub.region, sub.sales_rep_name
 )
ORDER BY 
 region;
